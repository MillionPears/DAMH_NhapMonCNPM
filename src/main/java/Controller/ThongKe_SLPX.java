...
public class ThongKe_SLPX {
    ...
    public ThongKe_SLPX() {
        ...
        cbb_year = dao.get_Year("PHIEU_XUAT", "NGAYXUAT");
        if (cbb_year.getItemCount() == 0) {
            JOptionPane.showMessageDialog(null, "Chưa tồn tại phiếu xuất nào!", "Lỗi ", JOptionPane.INFORMATION_MESSAGE);
    ...
    private CategoryDataset createDataset() {
        ...
        for (int i = 1; i <= 12; i++) {
            int total = dao.get_tongPX_theothang(i);
    ...
    public JFreeChart createChart() {
        JFreeChart barChart = ChartFactory.createBarChart(
                "TỔNG PHIẾU XUẤT THEO TỪNG THÁNG",
                "THÁNG", "SÓ LƯỢNG",
                createDataset());
        return barChart;
